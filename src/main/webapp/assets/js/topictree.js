    ...
    shadowOffset: {
      ...
      y : 0
		...
		_getComments: function(nodeid, version, topic) {
			var self = this;
			$.ajax({
				url: this.domain + "/json/comments/" + this.topic + "/" + nodeid + "/" + version,
				dataType: "json"
			}).done(function(json){
				self.comments = json;
				self._drawComments();
		...
		_newComment: function(comment) {
			...
			var downvotefunc = $.proxy(function(event){
				...
				var self = this;
				$.post(this.domain + "/json/vote/comment/" + id[2], "vote=down").done(function(data) {
			...
			var upvotefunc = $.proxy(function(event){
				...
				var id = parent.attr('id').split('-');
				var self = this;
				$.post(this.domain + "/json/vote/comment/" + id[2], "vote=up").done(function(data) {
					//self.comments.push(data.comment);
					//self._newComment(data.comment.id, data.comment.parent, "username", data.comment.text);
					//event.data.remove();
			...
			if($(".parent-id-" + comment.id).size() > 0) body.append($(".parent-id-" + comment.id));
			if($("#comment-id-" + comment.parent).size() > 0) {
  ...
  tween.play();*/
		...
		_newCommentReply: function(event) {
			...
			submit.click(form, $.proxy(function(event){
				var self = this;
				$.post(this.domain + "/json/add/comment", event.data.serialize()).done(function(data) {
					self.comments.push(data.comment);
		...
		_submitNewNode: function() {
			var self = this;
			$.post(this.domain + "/json/add/node", $('#new-node-form').serialize()).done(function(data) {
		...
		_submitNewVersion: function() {
			var self = this;
			$.post(this.domain + "/json/add/version", $('#new-version-form').serialize()).done(function(data) {
				self.data.push(data.node);
		...
		_submitNewComment: function() {
			var self = this;
			$.post(this.domain + "/json/add/comment", $('#new-comment-form').serialize()).done(function(data) {
				self.comments.push(data.comment);
