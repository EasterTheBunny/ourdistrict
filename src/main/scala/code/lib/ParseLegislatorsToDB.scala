...
object ParseLegislatorsToDB extends LiftActor {
  ...
  def messageHandler = {
    case ParseLegislators if !stopped => {
      ...
      val loc = Props.mode match {
        case Props.RunModes.Production => "/usr/local/govtrack/membership/legislators-current.yaml"
        case _ => "/Users/athughlett/Documents/GovTrack/membership/legislators-current.yaml"
      }
      val source = scala.io.Source.fromFile(loc)
      ...
      List(load.toArray(): _*).map(c => {
        ...
        val sp = Sponsor.find(By(Sponsor.thomas_id, sponsor("thomas").asInstanceOf[String])) match {
          case Full(s) => s
